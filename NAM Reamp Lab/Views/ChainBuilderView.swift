...
struct ChainBuilderView: View {
  ...
  private func chainContextMenu(for chain: ProcessingChain) -> some View {
    ...
    Button {
      ...
      Label(
        chain.isEnabled ? "Disable" : "Enable",
  ...
  private var workflowActionBar: some View {
    HStack(spacing: 16) {
      ...
      VStack(alignment: .leading, spacing: 2) {
        ...
        HStack(spacing: 6) {
          Circle()
            ...
            .frame(width: 8, height: 8)
          Text(
            "\(chainManager.enabledChains.count) chain\(chainManager.enabledChains.count == 1 ? "" : "s") enabled"
          )
          .font(.caption)
          .foregroundColor(.secondary)
        }
        ...
        modelNames = Dictionary(
          uniqueKeysWithValues:
            chainManager.enabledChains.map { chain in
              let isDefaultName =
                chain.name.hasPrefix("New Chain")
      ...
      .controlSize(.large)
      .disabled(
        chainManager.inputFileURL == nil || chainManager.enabledChains.isEmpty
          || chainManager.isProcessing
      )
  ...
  private func executeProcessAndTrain() async {
    ...
    for chain in chainManager.enabledChains {
      print(
        "   - \(chain.name): \(chain.plugins.count) plugins -> Model: \(modelNames[chain.id] ?? chain.name)"
      )
    }
