...
class NAMTrainer: ObservableObject {
  ...
  private init() {
    ...
    self.parameters = TrainingParameters(
      architecture: settings.trainingArchitecture,
      epochs: settings.trainingEpochs
    )
  }
  ...
  func createJob(
    ...
    let job = TrainingJob(
      ...
      modelName: modelName,
      chainName: chainName,
      parameters: self.parameters  // Capture current trainer parameters for this job
    )
    jobs.append(job)
    return job
  }
    ...
    do {
      ...
      if let index = jobs.firstIndex(where: { $0.id == job.id }) {
        ...
        Task { @MainActor in
          ...
          print(
            "✅ Set lastCompletedJob: \(jobs[index].chainName ?? "unknown") -> \(jobs[index].modelOutputPath ?? "no path")"
          )
        }
      }
    } catch {
  ...
  private func runTraining(for job: TrainingJob) async throws -> String {
    ...
    let trainingRunDir =
      "\(outputDir)/.training_runs/\(sanitizedModelName)_\(UUID().uuidString.prefix(8))"
    let modelOutputPath = "\(outputDir)/\(sanitizedModelName).nam"

    // Create directories
    try FileManager.default.createDirectory(atPath: outputDir, withIntermediateDirectories: true)
    try FileManager.default.createDirectory(
      atPath: trainingRunDir, withIntermediateDirectories: true)
    ...
    switch job.parameters.preset ?? .full {
    case .feather:
      channels = 4
    ...
    let modelConfig = """
      {
          ...
          "optimizer": {
              "lr": \(job.parameters.learningRate)
          },
          "lr_scheduler": {
              "class": "ExponentialLR",
              "kwargs": {
    ...
    let learningConfig = """
      {
          "train_dataloader": {
              "batch_size": \(job.parameters.batchSize),
              "shuffle": true,
              "pin_memory": false,
              "drop_last": true,
              "num_workers": 0
          ...
          "trainer": {
              "accelerator": "mps",
              "devices": 1,
              "max_epochs": \(job.parameters.epochs)
          },
          "trainer_fit_kwargs": {}
      }
      """
    try learningConfig.write(
      toFile: "\(configDir)/learning.json", atomically: true, encoding: .utf8)
        ...
        do {
          ...
          if process.terminationStatus == 0 {
            ...
            if let contents = try? fm.contentsOfDirectory(atPath: trainingRunDir) {
              let dateFolders = contents.filter { $0.contains("-") && !$0.contains(".") }.sorted()
                .reversed()
            ...
            continuation.resume(
              throwing: TrainingError.trainingFailed("Exit code: \(process.terminationStatus)"))
          }
        } catch {
  ...
  private func parseTrainingOutput(_ output: String, for job: TrainingJob) {
      ...
      let match = regex.firstMatch(in: output, range: NSRange(output.startIndex..., in: output))
    {
        ...
        let current = Int(output[currentRange])
      {
        ...
        jobs[index].progress = Double(current) / Double(jobs[index].parameters.epochs)
      }
    }
    ...
    if now.timeIntervalSince(lastLogUpdate) >= logUpdateInterval {
      ...
      let total = jobs[index].parameters.epochs
