...
class ChainManager: ObservableObject {
  ...
  @Published var selectedChainId: UUID? {
    willSet {
      // CRITICAL: Capture current state before switching to another chain
      if selectedChainId != newValue {
  ...
  func duplicateChain(_ chain: ProcessingChain) -> ProcessingChain {
    ...
    print(
      "✅ Duplicated chain '\(chain.name)' with \(copiedPlugins.count) plugins (including preset data)"
    )
    return newChain
  }
  ...
  func toggleChain(_ chain: ProcessingChain) {
    if let index = chains.firstIndex(where: { $0.id == chain.id }) {
      // Capture state first if it's the active one
      if selectedChainId == chain.id {
        captureCurrentChainState()
      }
  ...
  func renameChain(_ chain: ProcessingChain, to newName: String) {
    if let index = chains.firstIndex(where: { $0.id == chain.id }) {
      ...
      // CRITICAL: Synchronize with AudioEngine to prevent auto-save revert
      if AudioEngine.shared.currentChain?.id == chain.id {
        AudioEngine.shared.currentChain?.name = newName
      }
    }
  }
  ...
  func updateChainGroup(_ chain: ProcessingChain, to newGroup: String?) {
    if let index = chains.firstIndex(where: { $0.id == chain.id }) {
      let group =
        newGroup?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == true ? nil : newGroup
      chains[index].groupName = group

      // CRITICAL: Synchronize with AudioEngine to prevent auto-save revert
      if AudioEngine.shared.currentChain?.id == chain.id {
        AudioEngine.shared.currentChain?.groupName = group
      }
  ...
  /// Renames a chain based on the loaded presets of its plugins
  func renameChainFromPresets(_ chain: ProcessingChain) {
    // 1. Capture current states first to ensure we have the latest preset names
    captureCurrentChainState()

    // 2. Find the best candidate plugin for naming
    // Logic: NAM > Plugins with "Amp", "Drive", "Dist" in name > First plugin with a preset
    guard let latestChain = chains.first(where: { $0.id == chain.id }) else { return }
    let plugins = latestChain.plugins

    // Try NAM first
    if let namPlugin = plugins.first(where: { $0.type == .nam && $0.currentPresetName != nil }),
      let name = namPlugin.currentPresetName
    {
      renameChain(latestChain, to: name)
      return
    }

    // Try "Amp" plugins
    let ampKeywords = ["amp", "drive", "dist", "fuzz", "crunch"]
    if let ampPlugin = plugins.first(where: { plugin in
      let name = plugin.name.lowercased()
      return ampKeywords.contains { name.contains($0) } && plugin.currentPresetName != nil
    }), let name = ampPlugin.currentPresetName {
      renameChain(latestChain, to: name)
      return
    }

    // Fallback to any plugin with a preset
    if let bestPlugin = plugins.first(where: { $0.currentPresetName != nil }),
      let name = bestPlugin.currentPresetName
    {
      renameChain(latestChain, to: name)
    }
  }
  ...
  func removePlugin(_ plugin: AudioPlugin, from chain: ProcessingChain) {
    ...
    {
      // Capture state first if it's the active chain
      if selectedChainId == chain.id {
        captureCurrentChainState()
      }
  ...
  func movePlugins(in chain: ProcessingChain, from source: IndexSet, to destination: Int) {
    if let index = chains.firstIndex(where: { $0.id == chain.id }) {
      // Capture state before structural changes if it's the active chain
      if selectedChainId == chain.id {
        captureCurrentChainState()
      }
  ...
  func togglePlugin(_ plugin: AudioPlugin, in chain: ProcessingChain) {
    if let chainIndex = chains.firstIndex(where: { $0.id == chain.id }),
      let pluginIndex = chains[chainIndex].plugins.firstIndex(where: { $0.id == plugin.id })
    {
      // Capture state before reloading if it's the active chain
      if selectedChainId == chain.id {
        captureCurrentChainState()
      }
  ...
  func updatePluginNickname(_ plugin: AudioPlugin, in chain: ProcessingChain, nickname: String?) {
    ...
    guard let pluginIndex = chains[chainIndex].plugins.firstIndex(where: { $0.id == plugin.id })
    else { return }
  ...
  func captureCurrentChainState() {
    guard let selectedIndex = chains.firstIndex(where: { $0.id == selectedChainId }) else {
      return
    }

    // Get updated chain with plugin states from AudioEngine
    if let updatedChain = AudioEngine.shared.updateChainWithPluginStates() {
      // CRITICAL: Only update the plugin preset data. Overwriting the whole struct
      // would revert metadata like names, groups, and plugin nicknames if they
      // were changed in ChainManager but not yet synced to AudioEngine.
      for i in 0..<chains[selectedIndex].plugins.count {
        if i < updatedChain.plugins.count {
          chains[selectedIndex].plugins[i].presetData = updatedChain.plugins[i].presetData
          chains[selectedIndex].plugins[i].currentPresetName =
            updatedChain.plugins[i].currentPresetName
        }
      }

      // Update selectedChain (computed from chains array)
      objectWillChange.send()
      print("Captured plugin states for chain: \(chains[selectedIndex].name)")
    }
  }
  ...
  private func setupAutoSave() {
    ...
    // Periodic plugin state capture while running
    Timer.publish(every: 30, on: .main, in: .common)
      .autoconnect()
      .sink { [weak self] _ in
        guard let self = self else { return }
        guard
          AudioEngine.shared.isRunning && !self.isProcessing && !self.isEngineLoading
            && !NAMTrainer.shared.isTraining
        else { return }

        print("⏲️ Periodic plugin state capture...")
        self.captureCurrentChainState()
        self.saveChains(skipCapture: true)  // State already captured
      }
      .store(in: &cancellables)
  }
}
