...
struct TrainingJob: Identifiable, Codable, Equatable {
  ...
  init(
    ...
    self.modelName =
      modelName ?? chainName
      ?? URL(fileURLWithPath: outputFilePath).deletingPathExtension().lastPathComponent
...
struct TrainingParameters: Codable, Equatable {
  ...
  init(
    ...
    epochs: Int = 100,  // Match AppSettings default
...
extension TrainingJob {
  ...
  static let sampleRunning = TrainingJob(
    ...
    progress: 0.45,
    currentEpoch: 45,
    parameters: TrainingParameters(epochs: 100),
    currentLoss: 0.0023,
